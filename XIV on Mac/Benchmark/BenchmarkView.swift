...
struct BenchmarkView: View {
    @AppStorage(Benchmark.benchmarkFolderPref, store: .standard)
    var benchmarkFolder: String = ""
    ...
    func refreshCharacters() {
        availableCharacters = Benchmark.findAvailableDemoCharacters().sorted(
            by: { $0.id < $1.id })
    }
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                Picker(
                    selection: $benchMode,
                    label: Text("BENCHMARK_MODE_LABEL").font(.headline)
                ) {
                    Text("BENCHMARK_MODE_BENCHMARK_LABEL").tag(
                        BenchmarkMode.benchmark)
                    Text("BENCHMARK_MODE_CREATOR_LABEL").tag(
                        BenchmarkMode.characterCreator)
            ...
            HStack {
                Picker(
                    selection: $benchType,
                    label: Text("BENCHMARK_TYPE_LABEL").font(.headline)
                ) {
            ...
            if benchmarkVersion == .dawntrail && benchMode == .benchmark {
                VStack {
                    Picker(
                        selection: $selectedAppearanceSlot,
                        label: Text("BENCHMARK_CHARACTER_APPEARANCE").font(
                            .headline)
                    ) {
                        Text("BENCHMARK_DEFAULT_APPEARANCE").tag(nil as Int?)
                        ForEach(
                            availableCharacters,
                            content: { oneCharacter in
                                Text(oneCharacter.longDisplayName).tag(
                                    oneCharacter.id as Int?)
                            })
                    }
                    .onAppear {
                        refreshCharacters()
                    ...
                    Picker(
                        selection: $chosenCostume,
                        label: Text("BENCHMARK_CHARACTER_COSTUME").font(
                            .headline)
                    ) {
                        ForEach(BenchmarkCostumes.allCases, id: \.self) {
                            costume in
            ...
            HStack {
                ...
                Button("BENCHMARK_START_BUTTON") {
                    ...
                    Task {
                        ...
                        await Benchmark.launchFrom(
                            folder: benchmarkLocation, options: options,
